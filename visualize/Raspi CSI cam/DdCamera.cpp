//
// Video 4 Linux 2 CSI port interface to camera 
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// init camera using video 4 linux 2 library		
RET DdCamera::init(uint32_t width, uint32_t height, CAPTURE_FORMAT pixelFormat)
{
	...
	case CAPTURE_FORMAT_RGB332:
		v4l2pixelFormat = V4L2_PIX_FMT_RGB332; break;
	case CAPTURE_FORMAT_YVYU:
		v4l2pixelFormat = V4L2_PIX_FMT_YVYU; break;
	case CAPTURE_FORMAT_MJPEG:
		v4l2pixelFormat = V4L2_PIX_FMT_MJPEG; break;
	default:
	...
	m_fd = open(m_config.DD_CAMERA_V2L4, O_RDWR);
	if(m_fd < 0) {
