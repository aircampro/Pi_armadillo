 ...
 * gpioPinDc: 26
 */
Tsc2046Spi::Tsc2046Spi(const char* spiDevice, int gpioPinIrq, CALLBACK cb, void *arg)
{
...
// run the read thread
void* Tsc2046Spi::threadFunc(void *pInstance)
{
	Tsc2046Spi* p = (Tsc2046Spi*)pInstance;
	int fd;
	char str[64];
	struct pollfd pfd;
	...
	while(!p->m_isThreadExit) {
		...
		if(poll(&pfd, 1, 1000) != 0) {
			pressed = p->getPosition(&x, &y, &pressure);
			if(pressed) {
				p->m_cbTouch(x, y, pressure, p->m_cbArg);
...
bool Tsc2046Spi::getPosition(float *x, float *y, int *temp)
{
	uint8_t data0, data1;
	...
	*y = *temp / 2048.0f;
	...
	// measure pressure and return this value in temp
...
// creates the command byte to send to the touchpanel over spi port
uint8_t Tsc2046Spi::createCommand(uint8_t A, uint8_t mode, uint8_t SER)
{
	return 0x80 | (A << 4) | (mode << 3) | (SER << 2) | (0 << 0);
}

// command to read data
void Tsc2046Spi::sendCommand(uint8_t cmd, uint8_t *data0, uint8_t *data1)
{
...
// open write for GPIO controls
RET Tsc2046Spi::openWriteFile(const char* filePath, const char* str)
{
	int fd;
...
// init SPI
RET Tsc2046Spi::initializeSPI(const char* spiDevice)
{
	int ret = 0;
