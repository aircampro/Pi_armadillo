#
# This example python shows how to plot 2 y axis against a counter x
#
import time
import math
...
def main():
    ...
    plt.title('Plotting of 2 y axis data trends with time')
    ...
    try:
        ...
        while True:
            ...
            sign = randint(1,20)
            if sign <= 10:
                data.add(count, 30 + 1/randint(1,10) , 32 + randint(1,5))
            else:
                data.add(count, 30 - 1/randint(1,10) , 32 - randint(1,5))            
        ...
        print('\033[1m \nKeyboard interrupt received. closing Trend \033[0m')
        plt.close()
        ser.close()
        exit()
