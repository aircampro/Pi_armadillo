/*
    example test of openCV tracking algorithms
	compile as :- g++-8 -std=c++17 -o cv3_tracker_example cv3_tracker_example.cpp `pkg-config --cflags opencv` `pkg-config --libs opencv`
	usage : sudo ./cv3_tracker_example KCF video_name.mp4 if you want KCF algorithm
*/
...
int main(int argc, char** argv)
{
	...
	cv::String tracker_algorithm = parser.get<cv::String>(0);        // usage is e.g. sudo ./this_prog KCF video_name.mp4
	...
	// @@ Tracker::create(const cv::String& trackerType) for single tracker
	...
	while (true)
	{
		if (!Paused)
		{
			...
			if (SelectObject)
			{
				...
				// tracker->init(Image, BBox) for single tracker instance
				//
