...
async fn post_jpg2resnet_request(url: &str, payload: &str) -> std::io::Result<serde_json::Value> {
	let b64picture = base64::encode(payload).unwrap();
	let request = json!({
        'img' : &b64picture
    });
    let echo_json: serde_json::Value = reqwest::Client::new()
...
async fn post_txt_file_request(url: &str) -> std::io::Result<()> {
...
async fn post_jpeg_file_request(url: &str) -> std::io::Result<()> {
    let file = File::open("from_a_file.jpg")?;
	let b64picture = base64::encode(file).unwrap();
    let client = reqwest::Client::new();
    let res = client.post(url)
...
async fn post_jpeg_form_request(url: &str) -> std::io::Result<()> {
    let file = File::open("from_a_file.jpg")?;
	let b64picture = base64::encode(file).unwrap();
    let client = reqwest::Client::new();
    let res = client.post(url)
...
async fn post_png_form_request(url: &str) -> std::io::Result<()> {
    let file = File::open("from_a_file.png")?;
	let b64picture = base64::encode(file).unwrap();
    let client = reqwest::Client::new();
    let res = client.post(url)
...
async fn auth_request(url_specified: &str) -> std::io::Result<authResponse> {
    ...
    let res = client
        ...
        .send().await?
        .text().await
        .expect("Data acquisition failure");
...
async fn post_form_multipart_request(url: &str your_photo_path: &str, text_file: &str) -> std::io::Result<()> {
    let form = multipart::Form::new()
        // Adding just a simple text field...
        .text("username", "nick_o")
        // And a file...
        .file("photo", your_photo_path)?;
        .expect("your {} can't be opened",your_photo_path);
    
   // Customize all the details of a Part if needed...
   let bio = multipart::Part::text("customised detail part added")
       .file_name(text_file)
       .mime_str("text/plain")?;
