/*
  ...
  ref:- https://www.aptpod.co.jp/products/hardware/edgeplant-can-usb-if/
  ref:- https://github.com/aptpod/apt-peripheral-linux-driver/blob/main/samples/EP1-CH02A/read_can.c
  ref:- https://github.com/Kamioda/CommandLineParser/tree/master
  We use this device to read and write from the can bus via USB aptpod EP1CH02A device
  arg1 = usb device its attached too
  arg2 - baud rate
...
{
    /*
     * Set device path from the first commandline arg
     ...
     */
    CommandLineParser<char> args(argc, argv);
    for (const auto& arg : args) {
        // process args (we only do first one then break
        const char* devpath = arg;
        break;		
    }
	int br = args.getAs<int>(1);                     // get baudrate which is next arg as an integer
...

    {
        ...
        /*
         * Writes a CAN Frame buffer as below 
         * The data format of the CAN frame is as follows.
         * https://github.com/aptpod/apt-peripheral-linux-driver/blob/main/docs/EP1-CH02A.ja.md#can-frame
         */
        const unsigned char bufw[] = {
        ...
        /* Write CAN data */
        ssize_t wsize = write(fd, bufw, sizeof(bufw));
        if (wsize == -1) {
