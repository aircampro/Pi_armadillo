...
public:
	...
	void set_cam_fm()
    {
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("FM ends the action : " + name + "  ");
    }
	void set_cam_fa()
    {
        std::lock_guard<std::mutex> lock(m_mutex);	
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("FA ends the action : " + name + "  ");
    }	
	void set_cam_iso()
    {
        std::lock_guard<std::mutex> lock(m_mutex);	
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("ISO ends the action : " + name + "  ");
    }
	void set_cam_ss()
    {
        std::lock_guard<std::mutex> lock(m_mutex);	
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("SS ends the action : " + name + "  ");
    }
	void set_cam_sc()
    {
        std::lock_guard<std::mutex> lock(m_mutex);	
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("SC ends the action : " + name + "  ");
    }	
	void set_cam_expro()
    {
        std::lock_guard<std::mutex> lock(m_mutex);	
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(2));
        write_console("EX ends the action : " + name + "  ");
    }
};
...
int main(void)
{
	...
	//
    g_settings.set_ap = 50;
    g_settings.set_fa = 590;
    g_settings.set_fm = 530;
    g_settings.set_ss = 250;
    g_settings.set_sc = 450;
    g_settings.set_ex = 510;
    g_settings.set_iso = 650;
    g_settings.set_wb = 750;
	
	// now alternativly make a second set of objects calling one method using a class variable
	//
	...
	// set the class variables instead of the global structure
	//
    set_aperture1.set_value(50);
    set_wb1.set_value(750);
    set_fm1.set_value(530);
    set_fa1.set_value(590);
    set_iso1.set_value(650);
    set_ss1.set_value(250);
    set_sc1.set_value(450);
    set_expro1.set_value(510);	
