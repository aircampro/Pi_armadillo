...
boost_fs::path const dir("/home/anthony");
boost_fs::path const file("my_test.dat");
boost_fs::path const path = dir / file;

boost_fs::path const dir1("/home/anthony");
...
std::tuple< std::uintmax_t, std::uintmax_t, std::uintmax_t > get_pi_disk_usage() {

    const boost_fs::space_info si = boost_fs::space(path);
    std::cout
        << "│ " << static_cast<std::uintmax_t>(si.capacity) << ' '
        << "│ " << static_cast<std::uintmax_t>(si.free) << ' '
        << "│ " << static_cast<std::uintmax_t>(si.available) << ' '
        << "│ " << '\n';
    print_space("capacity", si.capacity);
    print_space("free", si.free);
    print_space("available", si.available);
    return std::make_tuple(get_mb(static_cast<std::uintmax_t>(si.capacity)), get_mb(static_cast<std::uintmax_t>(si.free)), get_mb(static_cast<std::uintmax_t>(si.available)));
}

std::uintmax_t free_space_main(void) {
    ...
    */
    ...
    // check for a file extension on a given path using boost algo
    //
    ...
    if (algo::contains(no_quo_path, ".d")) {
        std::cout << " contains .d " << "\n";
    }

    const boost_fs::space_info si = boost_fs::space(path);
    std::cout
        << "│ " << get_mb(static_cast<std::uintmax_t>(si.capacity)) << ' '
        << "│ " << get_mb(static_cast<std::uintmax_t>(si.free)) << ' '
        << "│ " << get_mb(static_cast<std::uintmax_t>(si.available)) << ' '
        << "│ " << '\n';
    std::tuple< std::uintmax_t, std::uintmax_t, std::uintmax_t > tup = get_pi_disk_usage();
    ...
    // this is an example of the write to file (it would be the global picture counter) but here i'm just giving it a value
...
void write_num_of_pics( int pic_count ) {
    std::ofstream ofs(path1);
    if (!ofs) {
        std::cerr << "failed to write counter file" << std::endl;
        std::exit(-1);
    }
    ofs << static_cast<std::intmax_t>(pic_count) << std::endl;
}

// read file stream to recall the photos counted total after reboot
// done in camera initialisation routine
//
std::uintmax_t read_num_of_pics() {
    std::ifstream ifs(path1);
    if (ifs.fail()) {
        std::cerr << "Failed to open counter file." << std::endl;
        std::exit(-1);
