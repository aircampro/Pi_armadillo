...
//#if defined(__focus_adjustment)
    ...
    if (g_gimbal_connected==1) {
        ...
        // this is an example showing how to send from serial port to gimbal
        //
...
//#endif
    ...
    if (xml_def_on == Settings_From_Ini) {
        ...
        if (g_gimbal_connected==1) {
#if defined(__focus_adjustment)
            r1 = write_camsets_to_json(ini_ap, ini_iso, ini_sc, ini_fm, ini_fa, ini_wb, ini_exp, ini_ss, sony_focus_settings.use_focus_setting);   // write the new set-up
#endif
        } else {
#ifndef __focus_adjustment
            r1 = write_camsets_to_json(ini_ap, ini_iso, ini_sc, ini_fm, ini_fa, ini_wb, ini_exp, ini_ss);   // write the new set-up
#else
            r1 = write_camsets_to_json(ini_ap, ini_iso, ini_sc, ini_fm, ini_fa, ini_wb, ini_exp, ini_ss, 0);   // write the new set-up	
#endif
        }
    } 
...
//#endif
    ...
    */
    ...
    if (sony_sc.my_value != sony_sc.my_prev_value) {
        std::uint32_t cam_enum_sc = enumerate_still_cap_sony_a7(sony_sc.my_value);
        if (camera->SetStillCaptureModeArgsInt(cam_enum_sc)==1) {
    ...
    for (;;) {
        ...
        for (auto z : v) {
            for (auto i : z) {
				...
				//sleep(1);
				usleep(2500000);
				if (camera->SetApertureArgsInt(i)==1) {
                   cli::tout << " success setting aperture " << i << "\n";
				   // update the object
                   sony_aper.add_new_value();	
                   // when you send the PARAM_EXT_ACK send this (would be in the other program
                   sony_aper.clr_update();		
        ...
        for (auto z : v) {
            for (auto i : z) {
				...
				//sleep(1);
				usleep(2500000);
				if (camera->SetWhiteBalanceArgsInt(i)==1) {
                   cli::tout << " success setting white balance " << i << "\n";
        ...
        for (auto z : v) {
            for (auto i : z) {
				...
				//sleep(1);
				usleep(2500000);
				if (camera->SetIsoArgsInt(i)==1) {
                   cli::tout << " success setting iso " << i << "\n";
				   // update the object
                   sony_iso.add_new_value();	
                   // when you send the PARAM_EXT_ACK send this (would be in the other program
                   sony_iso.clr_update();	
        ...
        for (auto z : v) {
            for (auto i : z) {
				...
				//sleep(1);
				usleep(2500000);
				if (camera->SetShutterSpeedArgsInt(i)==1) {
                   cli::tout << " success setting shutter speed " << i << "\n";
				   // update the object
                   sony_ss.add_new_value();	
                   // when you send the PARAM_EXT_ACK send this (would be in the other program
                   sony_ss.clr_update();	
...
//#endif
        ...
        if ((g_new_value == true) && (g_write_tdiff > NO_ACTIVITY_DURATION)) {
			...
			if (g_gimbal_connected==1) {
#if defined(__focus_adjustment)
                r1 = write_camsets_to_json(sony_aper.my_value, sony_iso.my_value, sony_sc.my_value, sony_foc_mode.my_value, sony_foc_area.my_value, sony_white_bal.my_value, sony_expro.my_value, sony_ss.my_value, sony_focus_settings.use_focus_setting);   // write the new set-up
#endif
            } else {
#ifndef __focus_adjustment
                r1 = write_camsets_to_json(sony_aper.my_value, sony_iso.my_value, sony_sc.my_value, sony_foc_mode.my_value, sony_foc_area.my_value, sony_white_bal.my_value, sony_expro.my_value, sony_ss.my_value);   // write the new set-up
#else
                r1 = write_camsets_to_json(sony_aper.my_value, sony_iso.my_value, sony_sc.my_value, sony_foc_mode.my_value, sony_foc_area.my_value, sony_white_bal.my_value, sony_expro.my_value, sony_ss.my_value, 0);	
#endif
            }
            g_new_value = false;
        }
