...
public:
    ...
    // simple lock mutex set/get value
    ...
    // these might be better methods than the simple lock guard
    void push(int dat)
    {
        boost::lock_guard<boost::mutex> lock(bv_mutex);              
        this->value = dat;                                           
        cond.notify_all();                                          
    }
    ...
    int pop()
    {
        boost::unique_lock<boost::mutex> lock(bv_mutex);            
        cond.wait(lock);                                                                                                          
        return this->value;
    }
...
int main(void)
{
    ...
    manager.set_sony_aper_event1(boost::bind(&Button::set_cam_aper1, &set_aperture1));
    manager.set_sony_wb_event1(boost::bind(&Button::set_cam_wb1, &set_wb1));
    manager.set_sony_fm_event1(boost::bind(&Button::set_cam_fm1, &set_fm1));
    manager.set_sony_fa_event1(boost::bind(&Button::set_cam_fa1, &set_fa1));
    manager.set_sony_iso_event1(boost::bind(&Button::set_cam_iso1, &set_iso1));
    manager.set_sony_ss_event1(boost::bind(&Button::set_cam_ss1, &set_ss1));
    manager.set_sony_sc_event1(boost::bind(&Button::set_cam_sc1, &set_sc1));
    manager.set_sony_expro_event1(boost::bind(&Button::set_cam_expro1, &set_expro1));
