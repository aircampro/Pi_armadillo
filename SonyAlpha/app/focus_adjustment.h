...
#include <ratio>
#include <chrono>
#include "mavlink_status_msg.cpp"
...
typedef struct {
    ...
    std::int16_t use_focus_setting;
    std::int16_t prev_focus_setting;
    std::uint16_t use_sdk_shut;
    std::uint16_t prev_sdk_shut;
    std::uint16_t use_sdk_fnum;
    std::uint16_t prev_sdk_fnum;
    std::uint16_t use_sdk_iso;
    std::uint16_t prev_sdk_iso;
    std::uint16_t mode_step : 4;
    std::uint16_t shut_step : 4;
    std::uint16_t iso_step : 4;
    std::uint16_t fnum_step : 4;
...
void init_focus_mode(sony_focus_settings_t* fs) {
    ...
    fs->shut_step = action_idle;
    fs->use_sdk_shut = 0;
    fs->prev_sdk_shut = 0;
    fs->fnum_step = action_idle;
    fs->use_sdk_fnum = 0;
    fs->prev_sdk_fnum = 0;
    fs->iso_step = action_idle;
    fs->use_sdk_iso = 0;
    fs->prev_sdk_iso = 0;
...
void check_focus_adjust(sony_focus_settings_t* fs, std::uint16_t chan_sig11, std::uint16_t chan_sig10, std::uint16_t is, std::uint8_t sid) {
    ...
    case modShutter:
    {
        ...
        if (chan_sig10 <= fs->sh_rc10fmu_spt2) {
            if ((SH_LOW_LIM < fs->use_sdk_shut) && (fs->shut_step != action_go_lo)) {
                fs->use_sdk_shut = --fs->use_sdk_shut;
                fs->shut_step = action_go_lo;
            }
        }
        else if (chan_sig10 >= fs->sh_rc10fmu_spt3) {
            if ((SH_HI_LIM > fs->use_sdk_shut) && (fs->shut_step != action_go_hi)) {
                fs->use_sdk_shut = ++fs->use_sdk_shut;
                fs->shut_step = action_go_hi;
            }
        }
        else {
            fs->shut_step = action_idle;
    ...
    case modAperture:
    {
        ...
        if (chan_sig10 <= fs->sh_rc10fmu_spt2) {
            if ((SH_LOW_LIM < fs->use_sdk_fnum) && (fs->fnum_step != action_go_lo)) {
                fs->use_sdk_fnum = --fs->use_sdk_fnum;
                fs->fnum_step = action_go_lo;
            }
        }
        else if (chan_sig10 >= fs->sh_rc10fmu_spt3) {
            if ((SH_HI_LIM > fs->use_sdk_fnum) && (fs->fnum_step != action_go_hi)) {
                fs->use_sdk_fnum = ++fs->use_sdk_fnum;
                fs->fnum_step = action_go_hi;
            }
        }
        else {
            fs->fnum_step = action_idle;
    ...
    case modIso:
    {
        ...
        if (chan_sig10 <= fs->sh_rc10fmu_spt2) {
            if ((SH_LOW_LIM < fs->use_sdk_iso) && (fs->iso_step != action_go_lo)) {
                fs->use_sdk_iso = --fs->use_sdk_iso;
                fs->iso_step = action_go_lo;
            }
        }
        else if (chan_sig10 >= fs->sh_rc10fmu_spt3) {
            if ((SH_HI_LIM > fs->use_sdk_iso) && (fs->iso_step != action_go_hi)) {
                fs->use_sdk_iso = ++fs->use_sdk_iso;
                fs->iso_step = action_go_hi;
            }
        }
        else {
             fs->iso_step = action_idle;
