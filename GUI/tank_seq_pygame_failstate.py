...
# make a word which selects the sprite in the event of equipment misalignment with the sequence of opeerations
def make_error_code_word():
    global E_CODE
    if ((SEQ_STATE == 0 and START_PB == 1) or SEQ_STATE == 5):
        E_CODE = not (inputs[0] == 0 and inputs[1] == 1) |  (not (inputs[2] == 0 and inputs[3] == 1)) << 1 |  not(inputs[4] == 0) << 2
    elif (STOP_PB == 1 or SEQ_STATE == 0) and instate == 0:
        E_CODE = not (inputs[0] == 0 and inputs[1] == 1) |  (not (inputs[2] == 0 and inputs[3] == 1)) << 1 |  not(inputs[4] == 0) << 2
    elif (STOP_PB == 1 or SEQ_STATE == 0) and instate == 1:
        E_CODE = not (inputs[0] == 1 and inputs[1] == 0) |  (not (inputs[2] == 0 and inputs[3] == 1)) << 1 |  not(inputs[4] == 0) << 2
    elif (SEQ_STATE == 1 or SEQ_STATE == 4):
        E_CODE = not (inputs[0] == 0 and inputs[1] == 1) |  (not (inputs[2] == 1 and inputs[3] == 0)) << 1 |  not(inputs[4] == 0) << 2
    elif (SEQ_STATE == 5):
        E_CODE = not (inputs[0] == 0 and inputs[1] == 1) |  (not (inputs[2] == 0 and inputs[3] == 1)) << 1 |  not(inputs[4] == 0) << 2
    elif (SEQ_STATE == 2):
        E_CODE = not (inputs[0] == 0 and inputs[1] == 1) |  (not (inputs[2] == 1 and inputs[3] == 0)) << 1 |  not(inputs[4] == 1) << 2
    elif (SEQ_STATE == 6 and inputs[5] == 1):
        E_CODE = not (inputs[0] == 1 and inputs[1] == 0) |  (not (inputs[2] == 1 and inputs[3] == 0)) << 1 |  not(inputs[4] == 1) << 2
    elif (SEQ_STATE == 6 and inputs[5] == 0):
        E_CODE = not (inputs[0] == 1 and inputs[1] == 0) |  (not (inputs[2] == 1 and inputs[3] == 0)) << 1 |  not(inputs[4] == 0) << 2
...
def handler(signum, frame):
    ...
    if inputs[0] == 0 and inputs[1] == 1 and inputs[2] == 0 and inputs[3] == 1 and inputs[4] == 0 and ((SEQ_STATE == 0 and START_PB == 1) or SEQ_STATE == 5):
        ...
        STATE_REACHED = 2
        FAIL_STEP = SEQ_STATE
        make_error_code_word()
    if not STATE_REACHED == 2:                                                                        # reset error code for graphic selection if there is no error
        E_CODE = 0
    signal_active = 0
...
def sequence():
    ...
    s0scale = pygame.transform.scale(s0, sz)
    step0_screens.append(s0scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step0_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step0_screens.append(s0scale)    
    ...
    s1scale = pygame.transform.scale(s1, sz)
    step1_screens.append(s1scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step1_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step1_screens.append(s0scale) 

    step2_screens = []    
    img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + "/step2.png" 
    s2 = pygame.image.load(str(img_path))
    s2scale = pygame.transform.scale(s2, sz)
    step2_screens.append(s2scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step2_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step2_screens.append(s0scale) 

    step3_screens = []         
    img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + "/step3.png" 
    s3 = pygame.image.load(str(img_path))
    s3scale = pygame.transform.scale(s3, sz)
    step3_screens.append(s3scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step3_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step3_screens.append(s0scale) 

    step6_screens = []        
    img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + "/step6.png" 
    s6 = pygame.image.load(str(img_path))
    s6scale = pygame.transform.scale(s6, sz)
    step6_screens.append(s6scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step6_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step6_screens.append(s0scale) 

    step6l_screens = []          
    img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + "/step6_low.png" 
    s6l = pygame.image.load(str(img_path))
    s6lscale = pygame.transform.scale(s6l, sz)
    step6l_screens.append(s6scale)
    # load the failure screens
    for i in range(1,8):
        img_path = os.path.dirname(os.path.abspath("__file__")) + "/images" + f"/step6_low_{i}.png" 
        s0 = pygame.image.load(str(img_path))
        s0scale = pygame.transform.scale(s0, sz)
        step6l_screens.append(s0scale)
    ...
    while ACTIVE == True:
        ...
        if SEQ_STATE == 0:                              # wait for start push button
            if instate == 1:
                screen.blit(step0_screens[E_CODE], (TPOSX, TPOSY))
            else:
                screen.blit(step1_screens[E_CODE], (TPOSX, TPOSY))  
            if STOP_PB == 1:
            ...
            if STOP_PB == 1:
                ...
                if STATE_REACHED == 1:
                    STATE_REACHED = 0
            if inputs[5] == 1:
                screen.blit(step6_screens[E_CODE], (TPOSX, TPOSY))
            else:
                screen.blit(step6l_screens[E_CODE], (TPOSX, TPOSY))            
