...
#           currently only supported is AM04142(16x120) and AM03128-H13
#
# ToDo:
...
#
# taken from here :- https://github.com/hanzov69/toledo
# adapted to run with linux raspi - aircampro
#
import sys
import getopt
import serial
...
def main(argv):
    ...
    global _stdout
     ...
     # default set-up    
    _port       = "/dev/ttyAMA0"                                               # device your attached to
    _message    = "Amplus Test use --message to set it"
    _line       = 1
    _font       = "1"
    _iEffect    = "5"
    _eEffect    = "5"
    ...
    for opt, arg in opts:
        ...
        if opt in ("-m", "--message"):
            ...
            toled()
        if opt in ("-o", "--output"):
            if (_message != "Amplus Test use --message to set it"):
                _output = 1
                toled()
            else:
