...
public:
  ...
  void start( float depth_near_value = 600.f, float depth_far_value = 1600.f )
  {
    ...
    thr = std::thread([this](){ this->capture(depth_near_value, depth_far_value); });
  }
  ...
  void transmit(time ts, int option = 18)
  {
	std::string s1,s2;
    cv::Mat dst1;
    if (mtx.try_lock())
