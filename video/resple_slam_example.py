...
# -*- coding: utf-8 -*-
#
# Example of SLAM Resple, I have shown how to add new items to the cost map which is published to ROS2
# RESPLE: Recursive Spline Estimation for LiDAR-Based Odometry
# ref:- https://github.com/YasiruDEX/RESPLE/tree/main
#
import numpy as np
import cv2 as cv
import rclpy
...
PLACES2LAND = ["Terrain", "Ground", "Other", "Building", "SideWalk", "Unlabeled", "Road"]
MIN_DIST = 0.01 # to avoid 1/zero

class CostMapGenNode(Node):

    def __init__(self, max_delta_t=0.1, RESX = 640, RESY = 480, CX = 320, CY = 240, FX = 432.455, FY = 432.455, QUAD_RADIUS = 0.5):
    ...
    def process_msgs(self, msg_dict):
        ...
        # get the items you want to add to the cost map (you can add here what you want from the LABELS
        ...
        # calculate the cost map for each item (all the ones you added above)
        ...
        # cost for water
        if water_mask.any():
            cost_map += self.get_heat((water_mask*255).astype('uint8'))
