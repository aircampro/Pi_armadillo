...
Stream multiple cameras in a loop through IPyCam.
...
def main_stream(cam_id: str):
    ...
    first_pass - False
    FPS = int(config.get(cam_id,'fps'))
    INDENT = '    '
    CAMERA_ID = int(config.get(cam_id,'id'))
    CAMERA_FPS = FPS
    CAMERA_WIDTH = int(config.get(cam_id,'image_width'))
    CAMERA_HEIGHT = int(config.get(cam_id,'image_height'))
    ...
    if not CAMERA_ID.find("RealsenseCapture") == -1:
        ...
        config2 = CameraConfig(name=f"My Depth Camera {uniq}",
            ...
            main_fps=30,
            main_bitrate="2M",
            main_stream_name: str = f"video_main_{uniq}_1"        
            # Substream settings
            sub_width=640,
            sub_height=360,
            sub_bitrate="512K",
            sub_stream_name: str = f"video_sub_{uniq}_1"
            # Hardware acceleration
            hw_accel="auto",                                                          # Options: "auto", "nvenc", "qsv", "cpu"
            # Network ports (optional, uses defaults if not specified)
            onvif_port=f"{onvif_start}",
        ...
        camera2 = IPCamera(config2)
        if not camera2.start():
            print("Failed to start camera2")
            return
    if not camera.start():
    ...
    if not CAMERA_ID.find("RealsenseCapture") == -1:
        try:
            ...
            cap.WIDTH = CAMERA_WIDTH
            cap.HEIGHT = CAMERA_HEIGHT
            cap.FPS = CAMERA_FPS
            cap.start()
        except:
        ...
        try:
            while camera.is_running:                                                            # Loop video
                ...
                if not CAMERA_ID.find("RealsenseCapture") == -1: 
                    color_frame = frame[0]
                    depth_frame = frame[1]  
                    cv2.putText(color_frame, f"Frame {frame_num}/{frame_count} {frame_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    camera.stream(color_frame)  
                    camera2.stream(depth_frame)                    
                else:
                ...
                if not k :
                    ...
                    if k.name == 'KEY_ESCAPE':
                        cap.release()
                        camera.stop()
                        if not CAMERA_ID.find("RealsenseCapture") == -1: camera2.stop()
...
def main(im_proxy):
    threadList = list()
    for _camra in ["camera1", "camera2", "camera3"]:                                          # list of your camera topics in the .ini file
        worker = threading.Thread(target=main_stream, args=(_camra))                          # create thread for each video file
