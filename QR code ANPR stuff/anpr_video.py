...
#
# ------------------------------------------------------------------------------
# Read ANPR car number plates using Yolo 
# -----------------------------------------------------------------------------
import argparse
import time
import re
...
# modbus interface to PLC we are client
#
import socket
...
def running_anpr(in_video, out_video, df, model, tracker, ocr):
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            for obj in tracked_objects:
              ...
              # hexlify the number plate recognised and send it to the PLC which operates the gates
              #
              hexnums = hexlify(text.encode('utf-8'))
              shift = 0
              addr = 10                                                     # start address in the modbus PLC
              for z in range(0, round(len(hexnums)/2)) :
                  val = int(hexnums,16) >> shift & 0xff                     # split the plate into each ascii char 16 bits long 
                  # send the val to the PLC
                  ret = writeHoldingRegisterModbus(cli, addr, val)          # write the plate as ascii numbers in decimal
                  print("set reg=",addr," value=",val)
                  shift += 8
                  addr += 1
        ...
        # when using a webcam press q key to exit
    ...
    try:
        ...
        print("cap already relased")

    disconnectModbusTCP(cli)        
...
parser.add_argument('--input', type=str, help='Path to the input video or 0 for webcamera')
