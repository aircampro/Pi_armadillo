...
""" Provides tools for general use. with ccTalk library modified to include checksum or CRC
...
class ChecksumOrCRC(object):

    def __init__(self, dataV):                   
        self.data = dataV
    ...
    def _validateChecksum(self):
        ...
        for byte in self.data[:-1]:
            total = total + ord(byte)
        return (256-(total%256)==ord(self.data[-1]))
    ...
    def _validateCRC(self):
        """
        Validates the CRC of a full message
        """
        crc = ord(self.data[2]) + (ord(self.data[-1]) << 8)
        self.data = self.data[0:2]+self.data[3:-1]
        return crc == self._calculateCRC()
...
def read_message(serial_object):
    ...
    # TODO: check checksum
    c = ChecksumOrCRC(reply)
    
    return reply, c._validateChecksum()
...
def send_message_and_get_reply(serial_object, message, verbose=False):
    ...
    output, fb1 = read_message(serial_object)
    reply, fb2 = read_message(serial_object)

    if fb2 == False:
        print("checksum invalid with reply")
        return False
