...
void opt_mark_0(void) { opt_mark(ord, 0); }                 // GND
void opt_mark_1(void) { opt_mark(ord, 1); }
void opt_mark_2(void) { opt_mark(ord, 2); }
void opt_mark_3(void) { opt_mark(ord, 3); }
void opt_mark_4(void) { opt_mark(ord, 4); }
...
#else
      if (i != 0) {
		  if (((tripc[i] - lst_tripc) < -EDIFF) && (tripc[i] < PMIN)) {                      // channel low
			wire_errors = wire_errors + 1.0f;  
          } else if ((((tripc[i] - lst_tripc) > EDIFF) && (lst_tripc < PMIN)) && (i==1)) {   // fitst channel low
