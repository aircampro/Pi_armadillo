...
filename = './tuning/tune_test_data_' + now.strftime('%Y%m%d_%H%M%S') + '.csv'                 # create tuning file with date and time in name 
...
class PidController():
    ...
    def __init__(self,
        ...
        self._i_mode: int = 2                                                     # mode to calculate integral term
        self._i_integrating: int = 0                                              # integrated total for i when i_mode=2
        self.imax: int = 10000                                                    # integral term max value
    ...
    def _integral(self, d_t: float, error: float) -> float:
        ...
        elif self._i_mode == 2:                                                             # this mode is with only the iterm accumulating and you use the term output PID standard.
            self._i_integrating += (1.0/self._ki) * d_t * error                             # i have chosen this to have constants from the auto Tune if not its 1/ki you use 
            self._i_integrating = np.clip(self._i_integrating, 0, self.imax)
            return self._i_integrating
...
PID_OT=0                                                                                     # choose output=1 or term=0 - use 0 for conventional PID for use with auto tuning file
...
if __name__ == "__main__": 
    ...
    # write header for tuning file
    with open(filename, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['t','u','y','ref'])
        ...
        try:    
            while 1:  
                ...
                if len(x) == round(30*PID_LOOP_TIME):                                           # we have 30 samples in 1second i.e 1s for 30sps
                    ...
                    # write a file for use with the autotuning PID_Tuner.py
                    #
                    # choice_of_op[PID_OT] = y
                    # t = time interval t
                    # str_avg = u
                    #
                    with open(filename, 'a', newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow([t, str_avg, choice_of_op[PID_OT], 1])       
