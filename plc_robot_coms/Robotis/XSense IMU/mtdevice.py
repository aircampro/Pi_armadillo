#!/usr/bin/env python3
# mtdevice.py
# adapted from  https://github.com/MrBoriska/xsense_imu/tree/master

# main code is reading Xsense AHRS data and doing filtering to produce a quaternion
# it is also driving 2 motors to move in a triangle/circle as below using 5 positions
#   
#            {X1 Y1} x     x
#       {X0 Y0} x               x {X3 Y0}
#
...
#                    x     x {X4 Y2}
#
# the motors are left/right and fwd/rev
import serial
import struct
import sys
import getopt
...
# using the kit library for madgwick
# 
from .quaternion import Quaternion
from .madgwick import MadgwickAHRS

# using munich library for ahrs
# pip install ahrs
from ahrs.filters.aqua import AQUA
from ahrs.filters.aqua import Davenport
from ahrs.filters.aqua import Mahony

# dynamixel drives
os.sys.path.append('../dynamixel_functions_py')             # Path setting
import dynamixel_functions as dynamixel                     # Uses DYNAMIXEL SDK library

# Control table address for YM80-230-R099-RH https://emanual.robotis.com/docs/en/dxl/y/ym080-230-r099-rh/
ADDR_PRO_OP_MODE       = 33                                 # operation mode
VEL_MODE = 1                                                # velocity
POS_MODE = 3                                                # position
...
ADDR_PRO_DIRECTION           = 32                           # paramter in the drive for setting drive to FWD or REV
FWD = 0
REV = 1
...
# Default setting for communication to drives
BAUDRATE                    = 1000000
...
DXL_VELOCITY_THRESHOLD = 20                                 # Dynamixel moving status threshold

COMM_SUCCESS                = 0                             # Communication Success result value
COMM_TX_FAIL                = -1001                         # Communication Tx Failed

dxl_goal_vel = 6972                                         # 69.72 rpm this is the max for the drive

import time
class Timer_Chk:
    def __init__(self, time_val, time_dl):
        self.time_v = time_val
        self.time_dly = time_dl
    def __eq__(self, other):
        return ((other.time_v - self.time_v) > other.time_dly)
...
################################################################
#
# this program reads the AHRS and moves round in a circle
#
def main():
    # parse command line
    ...
    try:
        ...
        usage()
        return 1
    # ======================= default configuration values for Xsense IMU ========================
    device = '/dev/ttyUSB0'  
    baudrate = 115200
    timeout = 0.002
    initial_wait = 0.1
    ...
    # action is a diamond like shape with these 4 co-ordinates the co-ord 0 is the start point
    posn_request = [(12,13), (12.3,13.3), (12.6,13), (12.6,12.7), (12.3,12.7)]
    # drive which externally runs when at the position for timer side_duration
    ext_run_drive = [2, 1, 2, 1]
    # 2 drives wheels fwd/rev and wheels left/right -- i.e. a box with 4 wheels (can use 1 drive with diff/gearbox)
    drives = [1, 2]
    
    # sequence of motion step
    step = 0
    last_step=0  
    step_duration=2 # traverse diagonly fwd-rev then left-right direction in steps of step_duration seconds
    side_duration=5 # move for side_duration seconds along the side after locking at positions 1-4
    latch_action=0  # set when moving on side_duration
    POSDB = 0.1     # position deadband (it is within +/- this value

    # set both drives to velocity control mode
    # for d in drives: 
    # set torque on both drives
    ...
    for d in drives:
        ...
        dynamixel.write1ByteTxRx(port_num, PROTOCOL_VERSION, d, ADDR_PRO_OP_MODE, VEL_MODE)
        ...
        dynamixel.write1ByteTxRx(port_num, PROTOCOL_VERSION, d, ADDR_PRO_TORQUE_ENABLE, TORQUE_ENABLE)
        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
    ...
    # if nothing else: set the requirement to echo the values
    if len(actions) == 0:
        actions.append('echo')
    try:
        if device == 'auto':
            ...
            if devs:
                ...
                # if there is more than one then please ask which one is the imu
                if len(devs) > 1:
                    print("which device do you want to use enter the number index starting at 0")
                    index_no = input()
                print("Using %s @ %d" % devs[index_no])
                device, baudrate = devs[index_no]
            else:
                print("No suitable device found.")
                return 1
        # find baudrate
        if not baudrate:
            baudrate = find_baudrate(device, timeout=timeout, verbose=verbose, initial_wait=initial_wait)
        if not baudrate:
            print("No suitable baudrate found.")
            return 1
        # open the xense imu device
        try:
            mt = MTDevice(device, baudrate, timeout=timeout, verbose=verbose, initial_wait=initial_wait)
        except serial.SerialException:
        ...
        if 'echo' in actions:
            ...
            mt.GoToConfig()            
            # place xense imu into measurement mode
            mt.GoToMeasurement()
            # call the madgwick class 
            madg = MadgwickAHRS()
            # use Roberto Valenti's Algebraic Quaterion Algorithm (AQUA)
            first_aq = 0
            # mahony
            first_mh = 0
            # davenport
            ...
            try:
                while True:
                    #
                    # show how to parse this data into variables you can use
                    #
                    op = mt.read_measurement(mode, settings)
                    print(op)
                    # raw data accelerometer,magnetrometer and gyro
                    # option r is mode |= OutputMode.RAW
                    acc_x = op['RAW']['accX']
                    acc_y = op['RAW']['accY']
                    acc_z = op['RAW']['accZ'] 
                    ...
                    mag_z = op['RAW']['magZ'] 
                    mag = [gy_x, gy_y, gy_z]
                    # call the madgewick AHRS compensation 
                    madg.update(gyro, acc, mag);
                    print("quaternion from AHRS RAW and madgwick = ", madg.quaternion)
                    # convert for use by the munich ahrs library   
                    acc_data = np.array([acc_x, acc_y, acc_z])  
                    gyr_data = np.array([gy_x, gy_y, gy_z])  
                    mag_data = np.array([mag_x, mag_y, mag_z])  
                    # call the valenti AQUA compensation                     
                    if first_aq == 0:
                       attitude = AQUA(acc=acc_data, mag=mag_data, gyr=gyro_data)
                       q = attitude.estimate(acc=acc_data, mag=mag_data)
                       q_new = attitude.updateIMU(q=q, gyr=gyro_data, acc=acc_data, mag=mag_data)                       
                       first_aq = 1
                    else:
                       q_new = attitude.updateIMU(q=q, gyr=gyro_data, acc=acc_data, mag=mag_data)
                       q = attitude.estimate(acc=acc_data, mag=mag_data)
                    print("quaternion from AHRS RAW and valenti AQUA = ", q_new)
                    # use Davenport filter
                    if first_dv == 0:
                       attitude = Davenport(acc=acc_data, mag=mag_data, gyr=gyro_data)
                       q = attitude.estimate(acc=acc_data, mag=mag_data)                      
                       first_dv = 1
                    else:
                       q = attitude.estimate(acc=acc_data, mag=mag_data)
                    print("quaternion from AHRS RAW and Davenport = ", q)  
                    # use Mahony filter
                    if first_mh == 0:
                       attitude = Mahony(acc=acc_data, mag=mag_data, gyr=gyro_data)
                       q = attitude.Q                      
                       first_mh = 1
                    else:
                       q = attitude.updateIMU(q=q, gyr=gyro_data, acc=acc_data, mag=mag_data)
                    print("quaternion from AHRS RAW and Davenport = ", q)                     
                    # quaternion from AHRS direct
                    # option is o for mode |= OutputMode.Orient
                    ahrs_quat = op['Orient']['quaternion']
                    print("quaternion from AHRS direct = ", ahrs_quat)
                    # raw gps data  
                    # option g is mode |= OutputMode.RAWGPS                    
                    lat = op['RAWGPS']['LAT']
                    lon = op['RAWGPS']['LON']
                    alt = op['RAWGPS']['ALT']   
                    print(" from GPRS lat %s lon %s alt %s" % (str(lat),str(lon),str(alt)))
                    # position direct
                    # option is p for mode |= OutputMode.Position 
                    plat = op['Pos']['Lat']
                    plon = op['Pos']['Lon']
                    palt = op['Pos']['Alt']   
                    print(" from AHRS lat %s lon %s alt %s" % (str(plat),str(plon),str(palt))) 
                    
                    # ====================== motion sequence in circle from position =================================
                    #
                    # use the posn to determine our motion sequence -- we must start by placing the bot at posn_request[0][0] + [0][1]
                    if ((abs(posn_request[step][0] - plat) < POSDB) and (abs(posn_request[step][1] - plon) < POSDB)) or (latch_action > 0): 
                        latch_action = 1
                        # traverse the side 
                        if ((step != 0) and (step <= 4)):                 # after 1 iteration of steps move along the side in one direction for side_duration
                            if latch_action == 1:
                                # stop both drives
                                for d in drives:
                                    dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, d, ADDR_PRO_GOAL_VELOCITY, 0)
                                    if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                                        dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                                    elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                                        dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                                # choose the motor to run along the side                                    
                                mtr_chose = ext_run_drive[(step-1)]
                                # run mtr_chose
                                dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, mtr_chose, ADDR_PRO_GOAL_VELOCITY, dxl_goal_vel)
                                if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                                    dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                                elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                                    dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                                run_side = Timer_Chk(time.time(),0)
                                latch_action = 2
                            elif latch_action == 2:
                                ran_for = Timer_Chk(time.time(), side_duration)                            
                                if (run_side == ran_for):
                                    #stop mtr_chose
                                    dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, mtr_chose, ADDR_PRO_GOAL_VELOCITY, 0)
                                    if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                                        dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                                    elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                                        dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                                    step = (step+1)                           # let step 5 decide if we are continous
                                    latch_action = 0
                        else:                                                 # @ start posn then just begin to move fwd and right in steps                                   
                            step = (step+1) % 5
                    # set drive directions if step changes (one-shot)
                    if ((step == 1) and not (last_step == step)): 
                        # drives directions drv1 F drv2 F
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[0], ADDR_PRO_DIRECTION, FWD)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[1], ADDR_PRO_DIRECTION, FWD)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        st_tm = Timer_Chk(time.time(),0) 
                        n = 0                        
                    elif ((step == 2) and not (last_step == step)): 
                        # drives directions drv1 R drv2 F
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[0], ADDR_PRO_DIRECTION, REV)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        ...
                        st_tm = Timer_Chk(time.time(),0)
                        n = 0 
                    elif ((step == 3) and not (last_step == step)): 
                        # drives directions drv1 R drv2 R
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[0], ADDR_PRO_DIRECTION, REV)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[1], ADDR_PRO_DIRECTION, REV)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        st_tm = Timer_Chk(time.time(),0)
                        n = 0 
                    elif ((step == 4) and not (last_step == step)): 
                        # drives directions drv1 F drv2 R
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[0], ADDR_PRO_DIRECTION, FWD)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                        ...
                        st_tm = Timer_Chk(time.time(),0)
                        n = 0 
                    elif ((step == 5) and not (last_step == step)): 
                        print("completed a lap of tank....press 1 to continue 0 exit")    # requires kb_input from operator to say another lap or quit
                        continue_flag = int(input())                        
                        if continue_flag == 1:
                            step = step % 5                         # reset sequence
                            n = 0 
                        elif continue_flag == 0:
                            exit()
                    last_step = step
                    # continously run the drive if not at a position and moving along one side
                    if (((step >= 1) and (step <= 4)) and (latch_action == 0)):
                        # start drive drives[n]
                        dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[n], ADDR_PRO_GOAL_VELOCITY, dxl_goal_vel)
                        if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                            dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                        elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                            dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                        act_tm = Timer_Chk(time.time(), step_duration)
                        if (st_tm == act_tm):
                            # stop drive drives[n]
                            dynamixel.write4ByteTxRx(port_num, PROTOCOL_VERSION, drives[n], ADDR_PRO_GOAL_VELOCITY, 0)
                            if dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION) != COMM_SUCCESS:
                                dynamixel.printTxRxResult(PROTOCOL_VERSION, dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION))
                            elif dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION) != 0:
                                dynamixel.printRxPacketError(PROTOCOL_VERSION, dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION))
                            st_tm = Timer_Chk(time.time(),0) 
                            n = (n+1) % 2                           # swap the drive
                                              
                    # velocity direct
                    # option is v for mode |= OutputMode.Velocity 
                    # The NED coordinate system, on the other hand, is a local-level coordinate system that is commonly 
                    # used for navigation purposes. It is defined with respect to a specific location or observer on the Earth's surface. 
                    # The origin of the NED coordinate system is typically set at the observer's position, and the NED axes are aligned such that:
                    ...
                    # The NED coordinate system is relative to the observer's position and orientation, while the inertial coordinate system is fixed with respect to the Earth
                    frm = op['Vel']['frame']
                    if (frm == 'NED'):
                        fram = 0
                    elif (frm == 'ENU'):
                    ...
                    vy = op['Vel']['Vel_Y']
                    vz = op['Vel']['Vel_Z']   
                    print(" from AHRS velocity frame %s x %s y %s z %s" % (frm,str(vx),str(vy),str(vz)))                      
            except Exception as e:
                print("exception ",e)            
            except KeyboardInterrupt:
                pass
            else:                                      # perfrom the action
