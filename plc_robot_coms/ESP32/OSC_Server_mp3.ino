...
// added for SPIFFS containing the mp3s to play from the OSC remote client
...
void messageCb(OSCMessage& m) {
  ...
  if (cmd == "talk") {
	...
	file = new AudioFileSourceSPIFFS(value.c_str());
  ...
  } else if (cmd == "volu") {                             // volume up
    g_gain = g_gaon + g_delta;
  } else if (cmd == "vold") {                             // volume down
    g_gain = g_gaon - g_delta;
  }
}
...
void setup() {
  ...
  // mp3 files are on spi filesystem  
  SPIFFS.begin();
