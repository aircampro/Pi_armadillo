...
def use_inverse_kinetics(s, pose_from):
    s.send(toBytes(
        "def myProg():"+"\n"
        "pose_tool="+"p["+str(pose_from[0])+","+str(pose_from[1])+","+str(pose_from[2])+","+str(pose_from[3])+","+str(pose_from[4])+","+str(pose_from[5])+"]"+"\n" # pose vector from 2nd position
        "get_inverse_kin(pose_tool, maxPositionError=1e-10, maxOrientationError=1e-10)" + "\n"
        +"end" +"\n")) 
    time.sleep(3)
    data_rcv = s.recv(1024)
    if (MY_PY == 3):
        data_rcv = data_rcv.decode('utf-8')
    if (len(data_rcv[0]) == 1):                                   # if a single array position is returned
        s.send(toBytes(
            "def myProg():"+"\n"
            "joint_pos="+str(data_rcv)+"\n"                       # joint positions from inverse kinetics
            "movej(joint_pos, a=1.4, v=1.05, t=0, r=0)" + "\n"
            +"end" +"\n"))    
    elif (len(data_rcv[0][0:1]) == 1):                            # if a multiple array position is returned take the first one
        s.send(toBytes(
            "def myProg():"+"\n"
            "joint_pos="+str(data_rcv[0])+"\n"                    # joint positions from inverse kinetics
            "movej(joint_pos, a=1.4, v=1.05, t=0, r=0)" + "\n"
            +"end" +"\n"))     
