...
# ref :- https://github.com/OpenCyphal-Garage/demos/blob/main/udral_servo/src/main.c
# added PID loop for temperature airCamPro 29/10/25
#
import os
import sys
import asyncio
...
class TempPID:
    ...
    def __init__(self, b=False, h=5.0, l=0, r=False) -> None:
        ...
        self._node = pycyphal.application.make_node(node_info, TempPID.REGISTER_FILE)
    ...
    async def _serve_execute_command(
        ...
        if request.command == uavcan.node.ExecuteCommand_1.Request.COMMAND_FACTORY_RESET:
            try:
                os.unlink(TempPID.REGISTER_FILE)  # Reset to defaults by removing the register file.
