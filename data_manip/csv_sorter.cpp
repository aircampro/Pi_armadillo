...
int main(int argc, char* argv[]) {
  ...
  std::string in_file = "input_data.csv");
  unsigned int time_col = 1;                                         // define the sort column as 1 (it starts @ 0)
  
  if (argc >= 2) {                                                   // parse command line for file and column to search
      try{
		  time_col = static_cast<unsigned int>(std::stoi(argv[1]));
          in_file = argv[2];
      } catch (...) {
          try {
		      time_col = static_cast<unsigned int>(std::stoi(argv[2]));
              in_file = argv[1];
          } catch (std::exception& e) {
              std::cout << " a standard exception was caught, with message '" << e.what() << "'\n";
              std::cout << "Example usage: " << argv[0] << " csvfilename.csv 2(column number you are sorting starting @ 0) " << std::endl;
          }
      }		  
  } else {
      std::cout << "Example usage: " << argv[0] << " csvfilename.csv 2(column number you are sorting starting @ 0 - using default values " << std::endl;  
  }
  
  // read the data from the input file
  std::vector<std::string> ss = ReadToStrVec(in_file);
  if (ss.size() == 0) {
	  return 2;
  }
  // create a vector of test data
  ...
  if (writing_file.fail()) {
	  std::cerr << "write file failed to open" << std::endl;
      std::quick_exit(4);
  }
  for (auto i : out) {
    std::cout << i << std::endl;
