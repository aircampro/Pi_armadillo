 ...
 * @note
 ...
 This is from the EGC over BLE monitoring project == https://www.vsigntek.com/1-about-vs-ecg/
 clone from this repository https://github.com/juangjl/vs-ecg.git and install as instructed
 ACP has modified it to write data to a sqllite database as well as displaying it as previously using GTK
 You need sqlite and GTK to run this project
...
void Form1LabelInfoUpdate(void)
{
	...
	///-----------------------------------------------------------------//
	/// Open database put the data and close the connection
	///-----------------------------------------------------------------//		
    sqlite3 *db;
    int rc, i, num;
    char *sql;
    ...
    sql = sqlite3_mprintf("create table if not exists ecg(date text not null, time text not null, temp float not null, \
    hr_now float not null, lead_off float not null, gsenxnow float not null, gsenynow float not null, gsenznow float not null, \
    fHrvSDNN float not null, fHrvNN50 float not null, fHrvRMSSD float not null, fHrvRR float not null, \
    fHrvVLF float not null, fHrvLF float not null, fHrvHF float not null, fHrvLFHF float not null,   \
    fHrvTP float not null, fHrvLFTP float not null, fHrvHFTP float not null, fBattTotalSec float not null,   )");
    /* Execute SQL statement */
    rc = sqlite3_exec(db, sql, NULL, NULL, NULL);

    /* transaction */
    sqlite3_exec(db, "begin", NULL, NULL, NULL);
	sprintf(strValue, "%04d/%02d/%02d",	pVscInfo->jtm.iYear, pVscInfo->jtm.iMonth, pVscInfo->jtm.iDay);
	sprintf(strValue1, "%02d:%02d:%02d", pVscInfo->jtm.iHour, pVscInfo->jtm.iMin, pVscInfo->jtm.iSec);										
    sql = sqlite3_mprintf(
        "insert into ecg (date, time, temp, hr_now, lead_off, gsenxnow, gsenynow, gsenznow  \
        fHrvSDNN, fHrvNN50, fHrvRMSSD, fHrvRR, fHrvVLF, fHrvLF, fHrvHF, fHrvLFHF, fHrvTP, fHrvLFTP, fHrvHFTP, fBattTotalSec ) values ('%s', %d)",
        strValue, strValue1, pVscInfo->fTemp, pVscInfo->fHRNow, pVscInfo->fLeadOff, pVscInfo->fGsenXNow \
        pVscInfo->fGsenYNow, pVscInfo->fGsenZNow, pVscInfo->fHrvSDNN, pVscInfo->fHrvNN50, pVscInfo->fHrvRMSSD, pVscInfo->fHrvRR \
		pVscInfo->fHrvVLF, pVscInfo->fHrvLF, pVscInfo->fHrvHF, pVscInfo->fHrvLFHF, pVscInfo->fHrvTP, pVscInfo->fHrvLFTP, pVscInfo->fHrvHFTP, pVscInfo->fBattTotalSec
    );
	...
	/// 1. time
	///-----------------------------------------------------------------//	
	sprintf(strValue, "%02d:%02d:%02d", pVscInfo->jtm.iHour, pVscInfo->jtm.iMin, pVscInfo->jtm.iSec); 
